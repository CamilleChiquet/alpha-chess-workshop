...
class OptimizeWorker:
    ...
    def start(self):
        ...
        if self.is_reinforcement_learning:
            self.model = self.load_model(True)
        else:
    ...
    def training(self):
        ...
        """
        if self.config.trainer.tensorboard_enabled and not self.is_reinforcement_learning:
        ...
        tmp_filenames = get_game_data_filenames(self.config.resource)
        if self.is_reinforcement_learning and len(tmp_filenames) > self.config.trainer.nb_recent_files_for_training:
            tmp_filenames = tmp_filenames[-self.config.trainer.nb_recent_files_for_training:]
        self.filenames = deque(tmp_filenames)
        ...
        while True:
            ...
            # We train only once for reinforcement learning pipeline
            if self.is_reinforcement_learning:
                break
    ...
    def compile_model(self, lr: float):
        ...
        # self.model.model.compile(optimizer=opt, loss=losses, loss_weights=[0.15, 1.0])
    ...
    def fill_queue(self):
        ...
        with ProcessPoolExecutor(max_workers=self.config.trainer.cleaning_processes) as executor:
            ...
            for _ in range(self.config.trainer.cleaning_processes):
                ...
                logger.debug(f"loading data from {filename} \t({len(self.filenames)} files remaining)")
